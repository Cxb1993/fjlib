...
//
//  Cation: matrix is colmajor, 'cause it corresponds to coordinates
//	1.6.2006 support generating unknown map matrix
//			generating and save ycenter_roots_x info 'cause both fill_vofs and fill_unknowns need it
//	2.21	changed get_ycenter_roots_x to get_y_roots_x and raise to public
//
*/
...
private:
	/// internal storage for ycenter_roots_x
	std::vector<vector_f>
					_ycrtsx;
...
//	void			get_ycenter_roots_x(size_t ys, vector_f& rts);
public:
	void			get_y_roots_x(float_t y, vector_f& rts);
protected:
	/// get ycenter roots x
	void			get_ycenter_roots_x_info();  
public:
	void			vof_gen()
	{ TFJVOFGen<CT,AT>::vof_gen(); get_ycenter_roots_x_info(); }
	virtual
	float_t			get_cell_vof(size_t m,size_t n);
	/// Calculate the vof values only
	virtual
	void			calc_vofs(matrix_f &mat);
	/// Fill the other vacant site
	virtual
	void			fill_vofs(matrix_f &mat);
	/// Fill the matrix with unknown info
	virtual
	void			fill_unknowns(matrix_n &mat);
