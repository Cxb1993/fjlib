...
void TFJVOFGen<ST,CT>::insert_line(size_t in, size_t jn)
{
	...
	do
	{
		...
		if (k!=(int)in) insert_pt(p,vrts[k]);
		k=inc(k);
	} while (k!=(int)inc(jn));
	TFJCurve_Line cl;
...
{
	size_t xsegs=TFJVOFGen<ST,CT>::xseg_count(),			// x=
		   ysegs=TFJVOFGen<ST,CT>::yseg_count();			// y=
	typename TFJVOFGen<ST,CT>::curve_type::interp_list_type&
		interps=this->curve->interps();		// helper

	xrts.resize(0);
	// get roots
	vector_f rts;
	if (!interps.y().splinx(
			this->axis->y().seg_center_value(ys),&rts,this->eps))
		return;
	...
	for (size_t i=0; i<rts.size(); i++)
		xrts[i]=this->axis->x().locate_seg(rts[i]);
}
*/

template <class ST, class CT>
void TFJVOFGen_Full<ST,CT>::get_y_roots_x(float_t y, vector_f& rts)
{
	typename TFJVOFGen<ST,CT>::curve_type::interp_list_type&
		interps=this->curve->interps();		// helper
	...
	rts.resize(0);
	float_t y0,y1;
	for (size_t i=0; i<this->curve->seg_count(); i++)
	{
		// make rought check to minimize higher order flucturation
		y0=this->curve->y(i);
		y1=this->curve->y(i+1);
		if ((y0-y)*(y1-y)<=0)
			interps.y().splinx(i,y,&rts,this->eps);
	}
	if (rts.size()<1) return; 
/*	
	// get roots
	if (!interps.y().splinx(y,&rts,this->eps))
		return;
*/
	size_t xsegs=TFJVOFGen<ST,CT>::xseg_count(),			// x=
		   ysegs=TFJVOFGen<ST,CT>::yseg_count();			// y=
...
/*
template <class ST,	class CT>
void TFJVOFGen_Full<ST,CT>::get_ycenter_roots_x(size_t ys, vector_f& rts)
{
}
*/
template <class ST,	class CT>
void TFJVOFGen_Full<ST,CT>::get_ycenter_roots_x_info()
{
	size_t yn=TFJVOFGen<ST,CT>::yseg_count();

	_ycrtsx.resize(yn);
	for (size_t j=0; j<yn; j++)
//		get_ycenter_roots_x(j,_ycrtsx[j]);
		get_y_roots_x(this->axis->y().seg_center_value(j),_ycrtsx[j]);
}

template <class ST,	class CT>
float_t TFJVOFGen_Full<ST,CT>::get_cell_vof(size_t m,size_t n)
{
	if (TFJVOFGen<ST,CT>::is_colored(m,n))
	{
		float_t sum=0.0;
		
		typename TFJVOFGen<ST,CT>::vof_curve_type& vc=
			TFJVOFGen<ST,CT>::vof_curve();
		...
		if (!cylinder)
			return  -sum/TFJVOFGen<ST,CT>::get_cell_area(m,n);
		else
			return -sum/2.0/this->axis->x().seg_center_value(m)/
						TFJVOFGen<ST,CT>::get_cell_area(m,n);
/*		
		if (!cylinder)
...
*/
	...
	else return this->void_value;
}

template <class ST,	class CT>
void TFJVOFGen_Full<ST,CT>::calc_vofs(matrix_f &mat)
{
	size_t m=TFJVOFGen<ST,CT>::xseg_count(),
			n=TFJVOFGen<ST,CT>::yseg_count();
	mat.resize(m,n);
...
template <class ST, class CT>
void TFJVOFGen_Full<ST,CT>::convert_xtoseg(const vector_f& v1,
											vector_n& v2)
{
	size_t xn=v1.size();
	v2.resize(xn);
...
{
	size_t xn=TFJVOFGen<ST,CT>::xseg_count();
	size_t yn=TFJVOFGen<ST,CT>::yseg_count();

	size_t cn=this->curve->pt_count();
	size_t ymax=this->axis->y().locate_seg(this->curve->y(0));
	size_t ymin=this->axis->y().locate_seg(this->curve->y(cn-1));
...
//		if (xrts.size()<1) { j++; continue; }
		...
		// convert xcoord to xseg
		if (_ycrtsx[j].size()<1) { j++; continue; }
		vector_n xrts;
		convert_xtoseg(_ycrtsx[j],xrts);
		...
		for (size_t i=0; i<xrts.size(); i++)
		{
			...
			if (_val)
				for (size_t k=m; k<=n; k++)
					if (mat(k,j)==this->void_value) mat(k,j)=1.0;
			_val=!_val;
		}
		j++;
	}
	...
	for (size_t m=0; m<mat.size1(); m++)
		for (size_t n=0; n<mat.size2(); n++)
			if (mat(m,n)==this->void_value) mat(m,n)=0.0;
}

template <class ST,	class CT>
void TFJVOFGen_Full<ST,CT>::fill_unknowns(matrix_n &mat)
{
	size_t xn=TFJVOFGen<ST,CT>::xseg_count();
	size_t yn=TFJVOFGen<ST,CT>::yseg_count();
	mat.resize(xn,yn);
